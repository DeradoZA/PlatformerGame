...
import { GameStateEnum, PlayerStateEnum } from './models/gameModels.js';
...
let jumpingGameObject = createNewJumpingGameObject();
// Initial Game Setup
showTextSequence([
...
function imgSize(img, fallback = { w: 25, h: 45 }) {
...
function computeCollisions() {
    ...
    for (const npc of npcObjectList) {
        ...
        if (npc.xPos + hitbox.w < playerObject.xPos)
            continue;
        const overlapX = Math.max(playerBox.xBoundaries[0], npcBox.xBoundaries[0]) <=
            Math.min(playerBox.xBoundaries[1], npcBox.xBoundaries[1]);
        const overlapY = Math.max(playerBox.yBoundaries[0], npcBox.yBoundaries[0]) <=
...
function handleObjectCollision(closestObject) {
...
function createNewJumpingGameObject() {
    return {
        gameDuration: 0,
...
async function showTextSequence(textSequence) {
...
function performGameStateChangeActions(newGameState) {
    switch (newGameState) {
